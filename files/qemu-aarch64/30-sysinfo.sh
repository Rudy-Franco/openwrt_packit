...
if [ "$PLATFORM" != "qemu-aarch64" ] && [ -x /usr/bin/cpustat ];then
    sys_temp=$(/usr/bin/cpustat -A)
else
    sys_temp=$(cat /proc/cpuinfo | grep name | cut -f2 -d: | head -n1)
    cpu_cnt=$(cat /proc/cpuinfo | grep name | wc -l)
    sys_temp="${sys_temp} x ${cpu_cnt}"
fi
